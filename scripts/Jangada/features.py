...
def mail2features(mail):
    ...
    for i, line in enumerate(mail):
        ...
        regex_special_works = "Dept\\.|University|Corp\\.|Corporations?|College|Ave\\.|Laboratory|[D|d]isclaimer|Division|Professor|Laboratories|Institutes?|Services|Engineering|Director|Sciences?|Address|Fax|Office|Mobile|Phone|Manager|Street|St\\.|Avenue"
        if i > 0:
        ...
        # email feature
        regex_email = "[^(\\<|\\>)][\\w|\\+|\\.|\\_|\\-]+\\@[\\w|\\-|\\_|\\.]+\\.[a-zA-z]{2,5}[^(\\<|\\>)]"
        if i > 0:
        ...
        if re.search("[^(\\<|\\>)][(\\w|\\+|\\_|\\-)]+\\@[(\\w|\\-|\\_)]+[\\.][a-zA-z]{2,5}", mail[i]):
            features['emailB'] = 1
            if (size - i) < tail_lines:
                features['closeemailB'] = 1

        # URL feature
        regex_url = "[\\s](http\\:\\/\\/)*(www|web|w3)*(\\w[\\w|\\-]+)\\.(\\w[\\w|\\-]+)\\.(\\w[\\w|\\-]+)*[\\w]+"
        if i > 0:
        ...
        if re.search("[A-Z][a-z]+\\s\\s?[A-Z][\\.]?\\s\\s?[A-Z][a-z]+", mail[i]):
            features['namepat'] = 1
            if (size - i) < tail_lines:
