...
We assume that each single line can be assigned to exactly one zone as \Fref{fig:examplemail} exemplary shows.
...
We compare results of our system described in \Fref{sec:model} with Jangada and Zebra.
...
As shown in \Fref{fig:model}, emails are processed in two stages: the line encoding and the email zoning stage.
...
In the initial stage of our system, the email text data is encoded into a low dimensional space to be used as input to the second stage as outlined on the left side in \Fref{fig:model}.
...
The convolutional model uses two convolutional layers, which scan the sequence of characters in a line and are intertwined by max-pooling and global-averaging layers finally leading into a densely connected layer, where the number of neurons corresponds to the embedding size as shown on the left in \Fref{fig:model}.
...
Thus, we added a second stage to our system for sequence to sequence classification using a GRU-CRF model as outlined in the right part of \Fref{fig:model}, which takes a sequence of line encodings per email as input.
...
\begin{figure}[tb]
	{%
		\pgfplotsset{x tick label style={font=\scriptsize}}
		\pgfplotsset{ylabel near ticks, xlabel near ticks}
		\pgfplotsset{every label/.append style={font=\scriptsize}}
		\pgfplotsset{xlabel={Maximum Line Length}, ylabel={Accuracy}}
		\pgfplotsset{width=\textwidth, height=0.8\textwidth}
	%	\pgfplotsset{/pgf/number format/.cd,fixed, fixed zerofill,precision=2}
		\pgfplotsset{y tick label style={/pgf/number format/.cd,fixed, fixed zerofill,precision=2}}%
	\centering
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\begin{tikzpicture} % 16: 48,49 | 32: 50,51 | 64: 52,53 (cnn,rnn|2zone)
		\begin{axis}[title=(a) Two-Zone Model,
	...
	\begin{subfigure}[t]{0.5\textwidth}
		...
		\begin{axis}[title=(b) Five-Zone Model,
	...
	\caption{Accuracy for increasing maximum line length using 32-dimensional embeddings}
	\label{fig:gridsearch}
\end{figure}

The ideal configuration for the line encoder model is determined through grid search across mentioned parameters.
...
The above models are baselines for the comparison to our proposed Quagga system as described in \Fref{sec:model} using a convolutional model as line encoder with fixed input sizes of 100 characters per line.
...
\begin{table}
	...
	\begin{tabular*}{0.8\textwidth}{@{\extracolsep{\fill}}cccc}
		\toprule
		Approach                & Zones & Enron & ASF\\ \midrule% & Others\footnote{Accuracy as reported in the original paper; Jangada only distinguishes between signature lines and "other", or additionally reply lines}\\ \midrule
...
\begin{figure}[tb]
	...
	{%
		%/pgf/number format/.cd,fixed,precision=2, set thousands separator={}, 
	%\pgfplotsset{trim axis left, trim axis right}
	\pgfplotsset{x tick label style={font=\scriptsize}}
	\pgfplotsset{ylabel near ticks, xlabel near ticks}
	\pgfplotsset{every label/.append style={font=\scriptsize}}
	\pgfplotsset{xlabel={Perturbation}, ylabel={Accuracy}, ymax=1.05}
	\pgfplotsset{width=\textwidth, height=0.8\textwidth}
	\pgfplotsset{/pgf/number format/.cd,fixed, fixed zerofill,precision=1}
	\centering
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		%\subcaption{Two Zones}
		\begin{tikzpicture} %
		\begin{axis}[title=(a) Two Zones,legend style={at={(0.01,0.65)},anchor=north west},
					 mark options={solid,scale=0.7}]
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		%\subcaption{Five Zones}
		\begin{tikzpicture} %
		\begin{axis}[title=(b) Five Zones,mark options={solid,scale=0.7}, 
	...
	\caption{Robustness against perturbation for the Enron test set}
